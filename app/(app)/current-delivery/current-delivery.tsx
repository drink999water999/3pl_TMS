...
import { driverAdvanceDispatch } from "../my-dispatches/actions";
...
export function CurrentDelivery({ item }: { item: Item | null }) {
  if (!item) {
    return (
      <Card className="p-10 text-center text-sm text-muted-foreground">
        You have no active delivery right now. When the office assigns you a
        trip it will show up here.
      </Card>
    );
  }
...
function DeliveryCard({ item }: { item: Item }) {
  ...
  // Trip details stay hidden until the driver marks the trip as dispatched.
  const detailsVisible = item.status !== "Assigned";

  // Delivery proof is always required to complete. Pickup proof is mandatory
  // ONLY for own-fleet trips, and must be uploaded before "Picked Up".
  const deliverNeedsPod = next === "Delivered" && !item.hasDeliveryPod;
  const pickupNeedsPod =
    next === "Picked Up" && item.isOwnFleet && !item.hasPickupPod;
  const blocked = deliverNeedsPod || pickupNeedsPod;
    ...
    <Card className="p-5">
      ...
      {detailsVisible ? (
        <>
          ...
          <div className="mt-2 space-y-1 text-xs text-muted-foreground">
            {item.pickupDate ? (
              <p>Pickup date: {formatDate(item.pickupDate)}</p>
            ) : null}
            {item.truck ? <p>Truck: {item.truck}</p> : null}
          </div>
        </>
      ) : (
        <p className="mt-4 rounded-md border border-dashed bg-muted/30 px-3 py-4 text-center text-sm text-muted-foreground">
          Trip details will appear once you tap{" "}
          <span className="font-medium text-brand-navy">Mark as Dispatched</span>
          .
        </p>
      )}
      ...
      {pickupNeedsPod ? (
        <p className="mt-3 rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-700">
          Own-fleet trip: upload a pickup photo before marking as picked up.
        </p>
      ) : null}
      ...
      {showPod ? (
        <PodForm
          dispatchId={item.id}
          defaultStage={item.isOwnFleet && !item.hasPickupPod ? "pickup" : "delivery"}
          onDone={() => {
            setShowPod(false);
            router.refresh();
          }}
